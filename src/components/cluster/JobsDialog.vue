...
<template>
  <v-dialog class="w-75" min-height="600" v-model="_is_open" @update:modelValue="$emit('update:is_open', false)">
    <v-card>
      <v-toolbar>
      ...
      <v-card-text>
        <v-switch v-model="filter_created_by_admin" color="primary">
          <template #label>
            ...
            <v-tooltip text="In order to show you live data this admin ui queries your CrateDB cluster every few seconds, some jobs shown here might not be relevant for you.">
              <template v-slot:activator="{ props }">
                <v-btn class="ml-3" v-bind="props" icon="mdi-information-outline" variant="text"></v-btn>
              </template>
            </v-tooltip>
          </template>
        </v-switch>
